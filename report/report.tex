...
\begin{description}
 ...
 \item[A set of rules] that can transform a state to another state.
...
\begin{description}
  ...
  \item[LEFT-ARC] Pop $w_{1}$ from $S$ and add the arc $(w_{2}, l, w_{1})$ to $T$, where $w_{2}$ is the first word in $B$. A precondition for this rule to be allowed to be applied is that $W_{1}$ is not the special word ROOT. This is to prevent the word ROOT from depending on any other words.

 \item[RIGHT-ARC] Pop $w_{1}$ from $S$, replace the first word $w_{2}$ in $B$ with $w_{1}$ and add the arc $(w_{1}, l, w_{2})$ to $T$.

 \item[SHIFT] Remove the first word $w$ in $B$ and push $w$ to $S$.
