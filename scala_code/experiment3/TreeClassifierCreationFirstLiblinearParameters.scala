...
object TreeClassifierCreationFirstLiblinearParameters extends Application{
...
""");
  ...
  def getTreeForFile(inputFile:String, divideByList:List[Int]):Tree = {
    ...
    val pTrain = Runtime.getRuntime().exec("./train -v 10 " + liblinearTrainingFileName)
    ...
    procentageCorrectLibLinear=try{
      ...
      case _:Exception => 0.0
       ...
       println("Children file names " + childrenFileNames.mkString(","))
       
       val childrenTmp = 
         if(index>1) 
          childrenFileNames.map(childFileName=>getTreeForFile(childFileName,divideByListNew))
         else
            Nil
