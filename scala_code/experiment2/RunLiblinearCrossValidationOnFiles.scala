 ...
 * 
 */
object RunLiblinearCrossValidationOnFiles {
  def main(args : Array[String]) : Unit = {
    ...
    while(new File(filesPrefix+index).exists){
     ...
     //Changed to "./train -s 4 -c 0.1 -v 10 " from "./train", "-v" ,"10",
    	val p = Runtime.getRuntime().exec("./train -s 4 -c 0.1 -v 10 " + fileToTestName + "lib_linear")
  ...
  case e:Exception=>println("Error when processing line " + line)
    ...
    println("Procent of total correct: " + (procentOfTotalCorrect*100) + " total count " + totalCount )
