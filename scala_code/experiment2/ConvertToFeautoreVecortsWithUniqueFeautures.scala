package experiment2

import java.io.{BufferedOutputStream, File, FileOutputStream, FileWriter}
import scala.io.Source
import java.util.Scanner
import java.io.File
/**
 * Arguments:
 * feauture_vecor_file cardinality_file
 * 
 */

object ConvertToFeautoreVecortsWithUniqueFeautures {
  def main(args : Array[String]) : Unit = {
    
    val feautureVecorFileSource = Source.fromFile(new File(args(0)))
    val cardinalityFileSource = Source.fromFile(new File(args(1)))
    
    val carArray = cardinalityFileSource.getLines().toList(0).split(",").map(_.trim).map(_.toInt)
    
    var columnCounterList = new Array[Int](0)
    

    
    var columnArgumentToNumberMapList = new Array[scala.collection.mutable.HashMap[String,Int]](0)
    var columns = 0
    
    //load everything from disk
    val feautureVecorFileSourceOpt = feautureVecorFileSource.getLines().toList
    
    
    for(line <- feautureVecorFileSourceOpt){
      val feautureVecorScanner = new Scanner(line.trim)
     // val cardinalityScanner = new Scanner(carString)
     // cardinalityScanner.useDelimiter(",")
      //Print the class
     
      print(feautureVecorScanner.nextInt() + "\t")
     
      var cardinalitySum = 1
      var columnNr = -1
      while(feautureVecorScanner.hasNext){
        columnNr += 1
        
        if(columnNr <= columns){
          columnCounterList = columnCounterList ++ Array(0)
          
          val hashMap = new scala.collection.mutable.HashMap[String,Int]()
          
          columnArgumentToNumberMapList = columnArgumentToNumberMapList ++ Array(hashMap)
          columns += 1
        }
        
        val columnValue = feautureVecorScanner.next()
        val columnCounter = columnCounterList(columnNr)
        val columnArgumentToNumberMap = columnArgumentToNumberMapList(columnNr)
        
        
        val id = columnArgumentToNumberMap.get(columnValue) match {
          case None => {
            columnCounterList(columnNr)=columnCounter+1
            columnArgumentToNumberMap.update(columnValue, (columnCounter+1))
            (columnCounter+1)
          }
          case Some(identifier) => identifier
        }
        
        
        print(id + cardinalitySum  + "\t")
        try{
        cardinalitySum += carArray(columnNr)
        }catch{
          case _ => ;
        }
      }
      
      println()
      
    }
    
    
    //exit(0)
  }
}
//ConvertToFeautoreVecortsWithUniqueFeautures.main(Array("fv","cf"))
